...
type DSpecialityRepository struct {
	pool *pgxpool.Pool
	db   *gorm.DB
}

func New(pool *pgxpool.Pool) *DSpecialityRepository {
	return &DSpecialityRepository{
		pool: pool,
	}
}

func (r *DSpecialityRepository) List(ctx context.Context, paging *model.Paging) ([]*reader.Dictionary, error) {
	var list []*reader.Dictionary
	var f = strings.Join(orm.Fields(reader.Dictionary{}).SqlFields(""), ",")

	var sql = "select " + f + " from nlrk_reader.d_speciality "
	if paging != nil {
		sql = sql + paging.Sql()
	}
