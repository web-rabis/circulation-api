...
type DEducationRepository struct {
	pool *pgxpool.Pool
	db   *gorm.DB
}

func New(pool *pgxpool.Pool) *DEducationRepository {
	return &DEducationRepository{
		pool: pool,
	}
}

func (r *DEducationRepository) List(ctx context.Context, paging *model.Paging) ([]*reader.Dictionary, error) {
	var list []*reader.Dictionary
	var f = strings.Join(orm.Fields(reader.Dictionary{}).SqlFields(""), ",")

	var sql = "select " + f + " from nlrk_reader.d_education "
	if paging != nil {
		sql = sql + paging.Sql()
	}
