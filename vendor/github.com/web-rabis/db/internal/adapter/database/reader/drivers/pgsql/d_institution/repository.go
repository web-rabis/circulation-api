...
type DInstitutionRepository struct {
	pool *pgxpool.Pool
	db   *gorm.DB
}

func New(pool *pgxpool.Pool) *DInstitutionRepository {
	return &DInstitutionRepository{
		pool: pool,
	}
}

func (r *DInstitutionRepository) List(ctx context.Context, paging *model.Paging) ([]*reader.Dictionary, error) {
	var list []*reader.Dictionary
	var f = strings.Join(orm.Fields(reader.Dictionary{}).SqlFields(""), ",")

	var sql = "select " + f + " from nlrk_reader.d_institution "
	if paging != nil {
		sql = sql + paging.Sql()
	}
