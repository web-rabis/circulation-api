...
type DAcademicDegreeRepository struct {
	pool *pgxpool.Pool
	db   *gorm.DB
}

func New(pool *pgxpool.Pool) *DAcademicDegreeRepository {
	return &DAcademicDegreeRepository{
		pool: pool,
	}
}

func (r *DAcademicDegreeRepository) List(ctx context.Context, paging *model.Paging) ([]*reader.Dictionary, error) {
	var list []*reader.Dictionary
	var f = strings.Join(orm.Fields(reader.Dictionary{}).SqlFields(""), ",")

	var sql = "select " + f + " from nlrk_reader.d_academic_degree "
	if paging != nil {
		sql = sql + paging.Sql()
	}
