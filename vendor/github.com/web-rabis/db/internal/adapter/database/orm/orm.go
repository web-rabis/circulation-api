...
func setValue(t reflect.Type, v reflect.Value, result pgx.Rows, fdm map[string]int, bson string, isPtr bool, mappingFunc MappingObjectsFn) {
	if t.Name() == "string" {
		...
		v1, _ := result.Values()
		v.Set(reflect.ValueOf(v1[fdm[bson]]))
		//i, err := strconv.ParseBool(string(result.RawValues()[fdm[bson]]))
		//if err == nil {
		//	v.Set(reflect.ValueOf(i))
		//}
	} else if t.Name() == "Time" {
		v1, err := result.Values()
		if err == nil && v1[fdm[bson]] != nil {
...
func (fm FieldsMap) FieldsValues(rf reflect.Value) ([]string, []any) {
	...
	for bson, field := range fm {
		...
		if rf.FieldByName(field.Info.Name).Type().String() == "time.Time" {
			fieldList = append(fieldList, bson)
			valueList = append(valueList, rf.FieldByName(field.Info.Name).Interface().(time.Time).Format("2006-01-02 15:04:05"))
		}
